...
@main_routes.route('/case/<int:case_id>')
def case_detail(case_id):
    case = next((case for case in cases if case['id'] == case_id), None)
    if case:
        img_path = os.path.join(current_app.config['UPLOAD_FOLDER'], case['image'])
        segmentation_path = os.path.join(current_app.config['UPLOAD_FOLDER'], case['segmentation'])
        ...
        overlay = None
        if 'overlay' in case:
            overlay_path = os.path.join(current_app.config['UPLOAD_FOLDER'], case['overlay'])
        ...
        if overlay:
            ...
            result_path = os.path.join(current_app.config['UPLOAD_FOLDER'], f'result_{case_id}.png')
...
@main_routes.route('/feature_examples')
def features():
    return render_template('feature_examples.html')


# Server side plotting example
@main_routes.route('/plot_sin', methods=['POST'])
def plot_sin():
