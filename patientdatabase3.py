...
EXTERNAL_DRIVE_PATH = '/Volumes/Seagate Bac/Thesis project 2025/database2.db'  # Update this path accordingly
...
class MRIFile(db1.Model):
    ...
    filename = db1.Column(db1.String(255), nullable=False)
    patient_id = db1.Column(db1.Integer, db1.ForeignKey('patient.id'), nullable=False)
    file_data = db1.Column(LargeBinary)
                ...
                if os.path.isdir(session_path):
                    ...
                    for csv_file in os.listdir(session_path):
                        if csv_file.endswith('.csv'):
                            ...
                            # For example, you might want to store specific data in the Patient model
                            # This is just a placeholder for your logic
                            # patient_entry.some_field = df['some_column'].values[0]  # Example
                        ...
                        for file in os.listdir(derivatives_session_path):
                            if file.endswith('.nii.gz'):
                                ...
                                if 'ctp' in file.lower():
                                    ctp_file = CTPFile(filename=file, patient_id=patient_entry.id, file_data=file_data)
                                    db1.session.add(ctp_file)
                                elif 'cta' in file.lower():
                                    cta_file = CTPFile(filename=file, patient_id=patient_entry.id,
                                                       file_data=file_data)  # Assuming CTA is also stored in CTPFile
                            ...
                            for file in os.listdir(perfusion_maps_path):
                                if file.endswith('.nii.gz'):
                                    ...
                                    if 'tmax' in file.lower():
                                        tmax_file = TMAXFile(filename=file, patient_id=patient_entry.id,
                                                             file_data=file_data)
                                        db1.session.add(tmax_file)
                                    elif 'mtt' in file.lower():
                                        mtt_file = MTTFile(filename=file, patient_id=patient_entry.id,
                                                           file_data=file_data)
                                        db1.session.add(mtt_file)
                                    elif 'cbv' in file.lower():
                                        cbv_file = CBVFile(filename=file, patient_id=patient_entry.id,
                                                           file_data=file_data)
                                        db1.session.add(cbv_file)
                                    elif 'cbf' in file.lower():
                                        cbf_file = CBFFile(filename=file, patient_id=patient_entry.id,
                                                           file_data=file_data)
                                        db1.session.add(cbf_file)

                        # Process ground truth files
                        for file in os.listdir(derivatives_session_path):
                            if file.endswith('.nii.gz') and 'ground_truth' in file.lower():
                                file_path = os.path.join(derivatives_session_path, file)
                                with open(file_path, 'rb') as f:
                                    file_data = f.read()  # Read the binary data
                                ground_truth_file = GroundTruthFile(filename=file, patient_id=patient_entry.id,
                                                                    file_data=file_data)
