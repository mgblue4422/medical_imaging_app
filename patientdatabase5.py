                    ...
                    for csv_file in os.listdir(session_path):
                        if csv_file.endswith('.csv'):
                            ...
                            # Read the CSV file into a DataFrame
                                ...
                                csv_file_entry = CSVFile(filename=csv_file, patient_id=patient_entry.id,
                                                         file_data=file_data)
                    ...
                    if os.path.isdir(derivatives_session_path):
                        ...
                        for file in os.listdir(derivatives_session_path):
                            if file.endswith('.nii.gz'):
                                ...
                                if 'ctp' in file.lower():
                                    ...
                                    db1.session.commit()  # Commit after adding the CTP file
                                    print(f"Added CTP file: {file}")  # Log the addition
                                elif 'cta' in file.lower():
                                    cta_file = CTAFile(filename=file, patient_id=patient_entry.id,
                                                       file_data=file_data)  # Assuming CTA is also stored in CTPFile
                                    db1.session.add(cta_file)
                                    db1.session.commit()  # Commit after adding the CTA file
                        ...
                        for file in os.listdir(derivatives_session_path):
                            if file.endswith('.nii.gz') and 'lesion-msk' in file.lower():
                                ...
                                db1.session.commit()  # Commit after adding the ground truth file
